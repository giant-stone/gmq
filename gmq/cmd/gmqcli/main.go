...
func main() {
	...
	// options
	flag.BoolVar(&optByForce, "f", false, "delete old message matched id(fulluri) before append")
	...
	if cmdPrintStats {
		...
		addMsg(ctx, broker, queueName, payloadStr, msgId, optByForce)
	} else if cmdGetMsg {
...
func addMsg(ctx context.Context, broker gmq.Broker, queueName, payloadStr, id string, byForce bool) {
	if payloadStr == "" && id == "" {
	...
	if optByForce {
		gutil.CheckErr(broker.DeleteMsg(ctx, queueName, id))
	}
